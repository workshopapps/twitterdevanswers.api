...
def fetch_user_username(username: str, db: Session = Depends(get_db), current_user: int = Depends(get_current_user)):
    """ Fetch a user by username  """

    user = crud.get_user_username(db, username=username)
    if not user:
        raise HTTPException(
