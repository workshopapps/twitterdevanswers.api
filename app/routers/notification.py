...
async def get_notifications(id: int, db: Session):
    ...
    number_of_unread = len([notification for notification in notifications if notification.unread == True])
...
def set_unread_to_false(id: int, db: Session):
    ...
    stored_notification = db.query(model.Notification).filter(model.Notification.notification_id == id).first()
...
async def notification_stream(request: Request, user_id: int = Depends(get_user_id), db: Session = Depends(get_db)):
    ...
    async def event_generator():
        while True:
            ...
            data = {"number_of_unread": number_of_unread, "notifications": notifications}
...
async def mark_read(
    notification_id: int = Path(default=..., description="The id of the notification to mark as read"),
    db: Session = Depends(get_db)
    ):
    """Sets the unread attribute of the specified notification to False."""
    result = set_unread_to_false(id=notification_id, db=db)
    if result is False:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Notification Id is invalid.")
    return {"success": True}
